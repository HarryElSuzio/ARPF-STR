...
--[[RegisterNetEvent("ARPF-EMS:str")
AddEventHandler("ARPF-EMS:str", function()
    Citizen.CreateThread(function()
        ...
        for k,v in ipairs(strHashes) do
...
RegisterCommand('spawnstr', function()
    if exports['ARPM']:checkdutystatus('emsduty') then
        LoadModel('prop_ld_binbag_01')
        local wheelchair = CreateObject(GetHashKey('prop_ld_binbag_01'), GetEntityCoords(PlayerPedId()), true)
    else
        exports['mythic_notify']:DoHudText('inform', 'You must be use /ems to use this command')
    end
end, false)
...
AddEventHandler("ARPF-EMS:togglestrincar", function()
    if exports['ARPM']:checkdutystatus('emsduty') then
        ...
        exports['mythic_notify']:DoHudText('error', 'You must be use /ems to use this command')
    end
end, false)
    ...
    if exports['ARPM']:checkdutystatus('emsduty') then
        ...
        if DoesEntityExist(closestObject) then
            ...
            if GetDistanceBetweenCoords(pedCoords, pickupCoords, true) <= 2.0 then
                exports['mythic_notify']:DoHudText('inform', 'Pushing Streatcher')
                PickUp(closestObject)
                --TriggerEvent("pickup", closestObject)
            end
        end 
    end
end)
